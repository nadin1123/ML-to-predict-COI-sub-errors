...
output=echinodermata-2%-lr-2ds-train-vs-test/output.txt
...
python3 ${SCRIPTDIR}/create_test_train_split_2datasets.py \
        --tag "echinodermata-2%-lr-2ds" -l "Label" \
        echinodermata_data/Echinodermata_3MinOrders_oh_2per.csv \
...
python3 ${SCRIPTDIR}/calculate_data_projection_lr.py \
                "echinodermata-2%-lr-2ds-train-vs-test"

# run LR on the data
python3 ${SCRIPTDIR}/evaluate_lr.py --fig "echinodermata-2%-lr-2ds-train-vs-test" | tee $output
