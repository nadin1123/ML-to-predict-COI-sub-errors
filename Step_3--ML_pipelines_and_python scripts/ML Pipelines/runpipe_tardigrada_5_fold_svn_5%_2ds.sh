...
# create the "folds"
python3 ${SCRIPTDIR}/create_folds_2datasets.py --tag "tardigrada-5%-svn-2ds" -l Label 5 tardigrada_data/Tardigrada_1MinOrder_oh_5per.csv tardigrada_data/Tardigrada_2MajOrder_oh_5per.csv
...
do
    ...
    output=tardigrada-5%-svn-2ds-folded-${fold}/output.txt

    # calculate the projection for each fold
    python3 ${SCRIPTDIR}/calculate_data_projection_svn.py "tardigrada-5%-svn-2ds-folded-${fold}"

    # run SVN on the fold
    python3 ${SCRIPTDIR}/evaluate_svn.py --classes "1,2" --fig "tardigrada-5%-svn-2ds-folded-${fold}" | tee $output
