...
# create the "folds"
python3 ${SCRIPTDIR}/create_folds_2datasets.py --tag "tardigrada-2%-lr-2ds" -l Label 5 tardigrada_data/Tardigrada_1MinOrder_oh_2per.csv tardigrada_data/Tardigrada_2MajOrder_oh_2per.csv
...
do
    ...
    output=tardigrada-2%-lr-2ds-folded-${fold}/output.txt

    # calculate the projection for each fold
    python3 ${SCRIPTDIR}/calculate_data_projection_lr.py "tardigrada-2%-lr-2ds-folded-${fold}"

    # run LR on the fold
    python3 ${SCRIPTDIR}/evaluate_lr.py --classes "1,2" --fig "tardigrada-2%-lr-2ds-folded-${fold}" | tee $output
