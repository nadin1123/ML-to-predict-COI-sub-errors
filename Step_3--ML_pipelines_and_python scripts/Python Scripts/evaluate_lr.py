...
##
## Use "grid search" to find good values for our LogisticRegression for this
## training set.
##
...
parameters = [
        ...
        {'C': [1, 10, 100, 1000], 'penalty': ['l2'], 'solver': ['newton-cg']}]

# perform the search
print(" . Performing grid search")
lr_search = GridSearchCV(
            LogisticRegression(),      # estimator object
...
print(" . Best parameters found for LogisticRegression:", best_parameters)



print(" . Creating LogisticRegression model to use on training data")
# Here the "**" means that we take the list of keyword=arg
# values in best_parameters and convert it into a dictionary
# to pass as arguments to the creation of the LogisticRegression implementation
...
outcome.to_csv('%s/lr_outcome.csv' % DATA_DIRNAME, index=False)
...
confusion.print_confusion_matrix(cm, CLASS_NAMES, y_training,
		filename="%s/lr_confusion.txt" % DATA_DIRNAME)

if MAKE_FIG:
	confusion.confusion_matrix_heatmap(
            '%s/lr_confusion_heatmap.pdf' % DATA_DIRNAME,
