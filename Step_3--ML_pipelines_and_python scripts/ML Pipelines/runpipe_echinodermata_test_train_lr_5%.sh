...
output=echinodermata-5%-lr-train-vs-test/output.txt
...
python3 ${SCRIPTDIR}/create_test_train_split.py \
        --tag "echinodermata-5%-lr" -l "Label" \
...
python3 ${SCRIPTDIR}/calculate_data_projection_lr.py \
                "echinodermata-5%-lr-train-vs-test"

# run SVN on the datea
python3 ${SCRIPTDIR}/evaluate_lr.py --fig "echinodermata-5%-lr-train-vs-test" | tee $output
