...
output=echinodermata-5%-rf-2ds-train-vs-test/output.txt
...
python3 ${SCRIPTDIR}/create_test_train_split_2datasets.py \
        --tag "echinodermata-5%-rf-2ds" -l "Label" \
        echinodermata_data/Echinodermata_3MinOrders_oh_5per.csv \
        echinodermata_data/Echinodermata_3MajOrders_oh_5per.csv

# calculate the projection for this split
python3 ${SCRIPTDIR}/calculate_data_projection_rf.py \
                "echinodermata-5%-rf-2ds-train-vs-test"

# run LR on the data
python3 ${SCRIPTDIR}/evaluate_rf.py --fig "echinodermata-5%-rf-2ds-train-vs-test" | tee $output
