...
python3 ${SCRIPTDIR}/create_test_train_split.py \
        --tag "tardigrada-2%-svn" -l "Label" \
        tardigrada_data/Tardigrada_2Orders_oh_2%.csv

# calculate the projection for this split
python3 ${SCRIPTDIR}/calculate_data_projection_svn.py \
		"tardigrada-2%-svn-train-vs-test" 

# run SVN on the datea
python3 ${SCRIPTDIR}/evaluate_svn.py --fig "tardigrada-2%-svn-train-vs-test"
