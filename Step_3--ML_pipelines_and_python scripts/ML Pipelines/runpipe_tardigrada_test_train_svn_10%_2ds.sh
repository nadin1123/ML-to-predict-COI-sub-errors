...
python3 ${SCRIPTDIR}/create_test_train_split_2datasets.py \
        --tag "tardigrada-10%-svn-2ds" -l "Label" \
        tardigrada_data/Tardigrada_1Order_oh_10%.csv \
	tardigrada_data/Tardigrada_2Orders_oh_10%.csv

# calculate the projection for this split
python3 ${SCRIPTDIR}/calculate_data_projection_svn.py \
		"tardigrada-10%-svn-2ds-train-vs-test" 

# run SVN on the datea
python3 ${SCRIPTDIR}/evaluate_svn.py --fig "tardigrada-10%-svn-2ds-train-vs-test"
