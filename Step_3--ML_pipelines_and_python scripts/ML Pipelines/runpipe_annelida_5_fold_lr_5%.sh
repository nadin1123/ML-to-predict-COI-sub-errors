...
# create the "folds"
python3 ${SCRIPTDIR}/create_folds.py --tag "annelida-5%-lr" -l Label 5 annelida_data/Annelida_2MajOrder_oh_5per.csv
...
do
    ...
    output=annelida-5%-lr-folded-${fold}/output.txt

    # calculate the projection for each fold
    python3 ${SCRIPTDIR}/calculate_data_projection_lr.py "annelida-5%-lr-folded-${fold}"

    # run LR on the fold
    python3 ${SCRIPTDIR}/evaluate_lr.py --classes "1,2" --fig "annelida-5%-lr-folded-${fold}" | tee $output
