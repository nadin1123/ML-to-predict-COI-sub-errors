...
# create the "folds"
python3 ${SCRIPTDIR}/create_folds_2datasets.py --tag "annelida-5%-rf-2ds" -l Label 5 annelida_data/Annelida_3MinOrder_oh_5per.csv annelida_data/Annelida_2MajOrder_oh_5per.csv
...
do
    ...
    output=annelida-5%-rf-2ds-folded-${fold}/output.txt

    # calculate the projection for each fold
    python3 ${SCRIPTDIR}/calculate_data_projection_rf.py "annelida-5%-rf-2ds-folded-${fold}"

    # run RF on the fold
    python3 ${SCRIPTDIR}/evaluate_rf.py --classes "1,2" --fig "annelida-5%-rf-2ds-folded-${fold}" | tee $output
