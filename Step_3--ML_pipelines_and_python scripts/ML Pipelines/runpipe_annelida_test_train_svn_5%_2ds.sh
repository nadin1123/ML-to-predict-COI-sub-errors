...
output=annelida-5%-svn-2ds-train-vs-test/output.txt
...
python3 ${SCRIPTDIR}/create_test_train_split_2datasets.py \
        --tag "annelida-5%-svn-2ds" -l "Label" \
        annelida_data/Annelida_3MinorOrders_oh_5per.csv \
        annelida_data/Annelida_2Orders_oh_5per.csv

# calculate the projection for this split
python3 ${SCRIPTDIR}/calculate_data_projection_svn.py \
                "annelida-5%-svn-2ds-train-vs-test"

# run LR on the data
python3 ${SCRIPTDIR}/evaluate_svn.py --fig "annelida-5%-svn-2ds-train-vs-test" | tee $output
