...
# create the "folds"
python3 ${SCRIPTDIR}/create_folds.py --tag "tardigrada-10%-rf" -l Label 5 tardigrada_data/Tardigrada_2MajOrder_oh_10per.csv
...
do
    ...
    output=tardigrada-10%-rf-folded-${fold}/output.txt

    # calculate the projection for each fold
    python3 ${SCRIPTDIR}/calculate_data_projection_rf.py "tardigrada-10%-rf-folded-${fold}"

    # run SVN on the fold
    python3 ${SCRIPTDIR}/evaluate_rf.py --classes "1,2" --fig "tardigrada-10%-rf-folded-${fold}" | tee $output
