...
# create the "folds"
python3 ${SCRIPTDIR}/create_folds_2datasets.py --tag "echinodermata-10%-lr-2ds" -l Label 5 echinodermata_data/Echinodermata_3MinOrders_oh_10per.csv echinodermata_data/Echinodermata_3MajOrders_oh_10per.csv
...
do
    ...
    output=echinodermata-10%-lr-2ds-folded-${fold}/output.txt

    # calculate the projection for each fold
    python3 ${SCRIPTDIR}/calculate_data_projection_lr.py "echinodermata-10%-lr-2ds-folded-${fold}"

    # run LR on the fold
    python3 ${SCRIPTDIR}/evaluate_lr.py --classes "1,2" --fig "echinodermata-10%-lr-2ds-folded-${fold}" | tee $output
