...
output=annelida-2%-lr-2ds-train-vs-test/output.txt
...
python3 ${SCRIPTDIR}/calculate_data_projection_lr.py \
                "annelida-2%-lr-2ds-train-vs-test"

# run LR on the data
python3 ${SCRIPTDIR}/evaluate_lr.py --fig "annelida-2%-lr-2ds-train-vs-test" | tee $output
