...
python3 ${SCRIPTDIR}/create_test_train_split.py \
        --tag "tardigrada-5%-lr" -l "Label" \
        tardigrada_data/Tardigrada_2Orders_oh_5%.csv

# calculate the projection for this split
python3 ${SCRIPTDIR}/calculate_data_projection_lr.py \
		"tardigrada-5%-lr-train-vs-test" 

# run SVN on the datea
python3 ${SCRIPTDIR}/evaluate_lr.py --fig "tardigrada-5%-lr-train-vs-test"
