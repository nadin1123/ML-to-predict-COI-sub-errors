...
X_train_and_validate, X_test, y_train_and_validate, y_test = \
        train_test_split(X, y, test_size=TESTSPLIT) # test_size represent the proportion of the dataset to include in the test split (0.25)
...
## wish to do, for example, parameter tuning.  We therefore need
...
# n_splits determines how many different validation (and training) sets you will create https://stackoverflow.com/questions/50772949/what-is-the-purpose-of-n-splits-in-sklean-model-selection-stratifiedshufflesplit
#test_size represent the proportion of the dataset to include in the training/validation split (0.5)
...
data_dirname = "%strain-vs-test" % TAG  #%s is a format specifier for or placeholders for formatting strings
if not os.path.exists(data_dirname):
    os.mkdir(data_dirname)

X_training.to_csv("%s/X_train.csv" % data_dirname, index=False) #index is for writing row names (set to False)
