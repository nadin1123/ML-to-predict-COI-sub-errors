...
output=echinodermata-2%-rf-2ds-train-vs-test/output.txt
...
python3 ${SCRIPTDIR}/create_test_train_split_2datasets.py \
        --tag "echinodermata-2%-rf-2ds" -l "Label" \
...
python3 ${SCRIPTDIR}/calculate_data_projection_rf.py \
                "echinodermata-2%-rf-2ds-train-vs-test"

# run LR on the data
python3 ${SCRIPTDIR}/evaluate_rf.py --fig "echinodermata-2%-rf-2ds-train-vs-test" | tee $output
