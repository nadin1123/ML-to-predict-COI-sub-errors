...
##
## Use "grid search" to find good values for our KNN for this
## training set.
##

# Set up the list of values to search among
parameters = [{'n_neighbors': [3, 5, 7, 9, 11, 13, 15], 'p': [1, 2], 'weights': ['uniform', 'distance'], 'metric': ['minkowski', 'euclidean', 'manhattan']}]
...
knn_search = GridSearchCV(
            KNeighborsClassifier(),      # estimator object
...
print(" . Best parameters found for KNN:", best_parameters)



print(" . Creating KNN model to use on training data")
# Here the "**" means that we take the list of keyword=arg
# values in best_parameters and convert it into a dictionary
# to pass as arguments to the creation of the KNN implementation
...
outcome.to_csv('%s/knn_outcome.csv' % DATA_DIRNAME, index=False)
...
confusion.print_confusion_matrix(cm, CLASS_NAMES, y_training,
		filename="%s/knn_confusion.txt" % DATA_DIRNAME)

if MAKE_FIG:
	confusion.confusion_matrix_heatmap(
            '%s/knn_confusion_heatmap.pdf' % DATA_DIRNAME,
