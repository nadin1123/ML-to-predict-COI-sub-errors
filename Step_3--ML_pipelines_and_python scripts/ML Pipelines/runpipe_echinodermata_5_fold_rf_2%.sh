...
# create the "folds"
python3 ${SCRIPTDIR}/create_folds.py --tag "echinodermata-2%-rf" -l Label 5 echinodermata_data/Echinodermata_3MajOrders_oh_2per.csv
...
do
    ...
    output=echinodermata-2%-rf-folded-${fold}/output.txt

    # calculate the projection for each fold
    python3 ${SCRIPTDIR}/calculate_data_projection_rf.py "echinodermata-2%-rf-folded-${fold}"

    # run SVN on the fold
    python3 ${SCRIPTDIR}/evaluate_rf.py --classes "1,2" --fig "echinodermata-2%-rf-folded-${fold}" | tee $output
