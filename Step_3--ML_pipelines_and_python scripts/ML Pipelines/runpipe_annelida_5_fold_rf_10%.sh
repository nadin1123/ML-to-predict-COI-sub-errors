...
# create the "folds"
python3 ${SCRIPTDIR}/create_folds.py --tag "annelida-10%-rf" -l Label 5 annelida_data/Annelida_2MajOrder_oh_10per.csv
...
do
    ...
    output=annelida-10%-rf-folded-${fold}/output.txt

    # calculate the projection for each fold
    python3 ${SCRIPTDIR}/calculate_data_projection_rf.py "annelida-10%-rf-folded-${fold}"

    # run RF on the fold
    python3 ${SCRIPTDIR}/evaluate_rf.py --classes "1,2" --fig "annelida-10%-rf-folded-${fold}" | tee $output
