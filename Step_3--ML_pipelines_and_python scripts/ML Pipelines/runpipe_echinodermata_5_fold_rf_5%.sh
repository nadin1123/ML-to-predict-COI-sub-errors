...
# create the "folds"
python3 ${SCRIPTDIR}/create_folds.py --tag "echinodermata-5%-rf" -l Label 5 echinodermata_data/Echinodermata_3MajOrders_oh_5per.csv
...
do
    ...
    output=echinodermata-5%-rf-folded-${fold}/output.txt

    # calculate the projection for each fold
    python3 ${SCRIPTDIR}/calculate_data_projection_rf.py "echinodermata-5%-rf-folded-${fold}"

    # run SVN on the fold
    python3 ${SCRIPTDIR}/evaluate_rf.py --classes "1,2" --fig "echinodermata-5%-rf-folded-${fold}" | tee $output
