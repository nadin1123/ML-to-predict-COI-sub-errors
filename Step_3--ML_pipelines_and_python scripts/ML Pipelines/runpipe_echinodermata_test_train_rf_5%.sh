...
output=echinodermata-5%-rf-train-vs-test/output.txt
...
python3 ${SCRIPTDIR}/create_test_train_split.py \
        --tag "echinodermata-5%-rf" -l "Label" \
...
python3 ${SCRIPTDIR}/calculate_data_projection_rf.py \
                "echinodermata-5%-rf-train-vs-test"

# run SVN on the datea
python3 ${SCRIPTDIR}/evaluate_rf.py --fig "echinodermata-5%-rf-train-vs-test" | tee $output
