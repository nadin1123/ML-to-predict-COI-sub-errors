...
output=echinodermata-10%-lr-2ds-train-vs-test/output.txt
...
python3 ${SCRIPTDIR}/create_test_train_split_2datasets.py \
        --tag "echinodermata-10%-lr-2ds" -l "Label" \
...
python3 ${SCRIPTDIR}/calculate_data_projection_lr.py \
                "echinodermata-10%-lr-2ds-train-vs-test"

# run LR on the data
python3 ${SCRIPTDIR}/evaluate_lr.py --fig "echinodermata-10%-lr-2ds-train-vs-test" | tee $output
